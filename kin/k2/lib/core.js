...
export const SUPA_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxtZnlzcG96Y215ZWZncmtneGNoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ2OTQxMTksImV4cCI6MjA5MDI3MDExOX0.cwB-4U5tnY3AC-lZhRv0ptWZZ5tqVZyK0TiHlQk1ViI';
...
export const COVER_MAL_IDS = [
  {id:21, title:'원피스'},      {id:20, title:'나루토'},
  {id:16498, title:'진격의 거인'}, {id:38000, title:'귀멸의 칼날'},
  {id:11061, title:'헌터×헌터'}, {id:5114, title:'강철의 연금술사'},
  {id:1535, title:'데스노트'},   {id:33, title:'베르세르크'},
  {id:40748, title:'주술회전'}, {id:44511, title:'체인소맨'},
  {id:223, title:'드래곤볼'},   {id:170, title:'슬램덩크'},
];
...
const KIN_REACTIONS = {
  ...
  streak_3:["어, 계속 맞히네. 이거 진짜야?","이거 진짜야? 혹시 전에 다 해봤어?","오. 계속 오네. 멈추지 말고."],
  streak_5:["이거 진짜 아는 거야? 찍는 거 아니지?","멈추지 않네. 솔직히 좀 무서워지기 시작했어.","좀 무서운데. 어디까지 가는 거야."],
  streak_7:["...인정하기 싫은데. 인정해야 할 것 같아.","이 정도면 단골이야. 그냥 여기 살아.","어, 진짜로? 이건 나도 예상 못 했어."],
};
...
const KIN_REACTIONS_EN = {
  ...
  streak_3:["Oh, you keep getting them. Is this real?","Is this real? Did you do this before?","Oh. Keep going. Don't stop now."],
  streak_5:["Do you actually know this? You're not guessing, right?","You're not stopping. Honestly starting to get a little scary.","Getting a bit scary. How far does this go."],
  streak_7:["...I don't want to admit it. But I have to.","You're a regular. You might as well just live here.","Oh, for real? Even I didn't see this coming."],
};
...
const RESULT_LABELS = {
  ko:{
    perfect:'만화방 단골', zero:'용기 있는 입문자',
    hard_easy_gap:'이상한 덕후', major_only:'유명작 생존자',
    outlier_strong:'소거법의 달인', one_work:'원작 충성파',
    author_strong:'작가 추종형', relation_strong:'커플링형',
    easy_only:'입문 완료', all_high:'생각보다 진짜 본 사람',
    all_low:'입문자', random:'무작위 독자',
  },
  en:{
    perfect:'Manga cafe regular', zero:'Brave beginner',
    hard_easy_gap:'Oddly specific fan', major_only:'Mainstream survivor',
    outlier_strong:'Process of elimination', one_work:'Loyal to one',
    author_strong:'Author follower', relation_strong:'Shipping everything',
    easy_only:'Intro complete', all_high:'Actually watched it',
    all_low:'Just starting out', random:'Random reader',
  },
};

export function getLabel(pattern) {
  return (getLang() === 'ko' ? RESULT_LABELS.ko : RESULT_LABELS.en)[pattern]
...
const OBS = {
  ...
  en:{
    ...
    all_high:["You've read widely. Either no taste or all taste. Probably all taste.","Even across everything. You read without bias.","No genre preference. You'd be the last one to leave a manga cafe."],
...
export function getReplayHint(pattern) {
  ...
  return map[pattern] || (getLang() === 'ko' ? '레이블이 바뀔 수도 있어.' : 'Your label might change.');
}
...
export function detectPattern(answers) {
  ...
  const mw = Object.values(wc).reduce((b, w) =>
    w.total >= 3 && w.correct / w.total > (b.correct / Math.max(b.total, 1)) ? w : b,
    { total: 0, correct: 0 }
  );
